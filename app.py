...
def load_and_process_data():
    """
    Carrega o arquivo do repositório, padroniza as colunas dos 3 anos (2022-2024)
    e consolida a base analítica do PEDE.
    """
    file_path = "PEDE_PASSOS_MAGICOS.xlsx"

    try:
        xls = pd.ExcelFile(file_path)
    except Exception as e:
        st.error(f"Erro ao carregar o arquivo '{file_path}'. Verifique se ele foi enviado para a raiz do repositório no GitHub.")
        st.stop()

    frames = []

    # Mapas de padronização por aba
    cols_2022 = {
        'RA': 'RA', 'NOME': 'Nome', 'TURMA': 'Turma', 'FASE': 'Fase', 'PEDRA': 'Pedra',
    ...
    for sheet, (year, col_map) in map_years.items():
        if sheet in xls.sheet_names:
            ...
            # Evita o FutureWarning tratando colunas ausentes de forma explícita
            if 'IPP' not in df_temp.columns:
                df_temp['IPP'] = pd.Series(dtype='float64')
            if 'Atingiu_PV' not in df_temp.columns:
                df_temp['Atingiu_PV'] = None

            keep_cols = ['RA', 'Nome', 'Turma', 'Fase', 'Pedra', 'Ano', 'INDE', 'IAN', 'IDA',
    ...
    # Classificação do IAN / Defasagem Idade-Série
...
fases_disponiveis = sorted([f for f in df['Fase'].dropna().unique() if str(f).strip() != ''])
selected_fases = st.sidebar.multiselect("Fases Pedagógicas", fases_disponiveis, default=fases_disponiveis)

pedras_disponiveis = [p for p in df['Pedra'].dropna().unique() if str(p).strip() != '']
...
with tab_defasagem:
    ...
    st.caption("Evolução da distribuição dos alunos por nível de defasagem pedagógica ao longo dos anos[cite: 4].")
...
with tab_engajamento:
    ...
    with col_e2:
        ...
        if not df_pv.empty:
            ...
            st.info("Para visualizar a comparação oficial do Ponto de Virada, certifique-se de incluir o ano de 2022 nos filtros laterais[cite: 4].")
...
st.markdown(
    ...
    "Dashboard Passos Mágicos | Plataforma de Análise de Impacto Social"
    "</div>",
    unsafe_allow_html=True
)