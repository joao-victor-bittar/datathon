...
def load_and_process_data():
    """
    Busca o arquivo Excel usando o caminho absoluto baseado no diretório do app.py,
    padroniza as colunas dos 3 anos (2022-2024) e consolida a base analítica do PEDE.
    """
    # Garante que vai achar o arquivo na mesma pasta onde o app.py está instalado
    BASE_DIR = Path(__file__).resolve().parent
    file_path = BASE_DIR / "BASE DE DADOS PEDE 2024 - DATATHON.xlsx"

    # Se por acaso não achar no diretório do script, tenta na raiz de trabalho
    ...
    # Tratamento de tipos nas colunas de filtro para evitar erros de ordenação
    df_pede['Fase'] = df_pede['Fase'].astype(str).str.strip()
    df_pede['Pedra'] = df_pede['Pedra'].astype(str).str.strip()
...
# Correção na ordenação e filtro das Fases e Pedras
fases_disponiveis = sorted([f for f in df['Fase'].dropna().unique() if f not in ['', 'nan', 'None']], key=str)
selected_fases = st.sidebar.multiselect("Fases Pedagógicas", fases_disponiveis, default=fases_disponiveis)

pedras_disponiveis = sorted([p for p in df['Pedra'].dropna().unique() if p not in ['', 'nan', 'None']], key=str)
